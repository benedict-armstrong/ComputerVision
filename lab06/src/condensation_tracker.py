...
def condensation_tracker(video_name, params):
    ...
    for i in range(last_frame - first_frame + 1):
        ...
        if params["draw_plots"]:
            ...
            new_plot = ax.scatter(
                particles[:, 0], particles[:, 1], color='blue', s=2)
            ...
            for j in range(i-1, -1, -1):
                ...
                if lwidth > 0:
                    new_plot = ax.scatter(
                        mean_state_a_priori[j+1, 0], mean_state_a_priori[j+1, 1], color='blue', s=lwidth)
                    to_remove.append(new_plot)
                if j != i:
        ...
        if params["draw_plots"]:
            ...
            for j in range(i-1, -1, -1):
                ...
                if lwidth > 0:
                    new_plot = ax.scatter(
                        mean_state_a_posteriori[j+1, 0], mean_state_a_posteriori[j+1, 1], color='red', s=lwidth)
                    to_remove.append(new_plot)
                if j != i:
