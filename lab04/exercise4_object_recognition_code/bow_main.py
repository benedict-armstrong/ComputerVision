...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    """
    x, y = np.meshgrid(
        np.linspace(border, img.shape[1] - border, nPointsX),
        np.linspace(border, img.shape[0] - border, nPointsY)
    )
...
def test_grid_points():
    # test grid points with a sample image
    img = cv2.imread(
    ...
    for i in range(vPoints.shape[0]):
        cv2.circle(img_out, (int(vPoints[i, 0]), int(
            vPoints[i, 1])), 1, (0, 255, 0), -1)

    cv2.imwrite('grid_points.png', img_out)


def descriptors_hog(img: np.array, vPoints: np.array, cellWidth: int, cellHeight: int):
    nBins = 8
    w = cellWidth
    h = cellHeight
...
def test_descriptors_hog():
    # test descriptors_hog with a sample image
    img = cv2.imread(
        "data/data_bow/cars-testing-neg/image_0051.png")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    vPoints = grid_points(img, 20, 20, 30)
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    cached_file = os.path.join(CACHE_DIR, f"codebook_{k}_{numiter}.npy")

    if os.path.isfile(cached_file) and use_cache:
        print("Loading cached features")
        vFeatures = np.load(cached_file)
    else:
        print("No cached features found, computing")
        ...
        for i in tqdm(range(nImgs)):
            ...
            # todo
            vPoints = grid_points(img, nPointsX, nPointsX, border)
...
def bow_histogram(vFeatures: np.array, vCenters: np.array):
    """
...
def create_bow_histograms(nameDir, vCenters: np.array):
    """
    ...
    vBoW = np.zeros((nImgs, vCenters.shape[0]))
    for i in tqdm(range(nImgs), desc="Hist"):
        # print('processing image {} ...'.format(i + 1))
        img = cv2.imread(vImgNames[i])  # [172, 208, 3]
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # [h, w]
        ...
        # todo
        histo = bow_histogram(descriptors, vCenters=vCenters)
        vBoW[i, :] = histo
...
def bow_recognition_nearest(histogram, vBoWPos, vBoWNeg):
    ...
    # Distance as sum of all distances
    DistPos = np.sum(np.abs(histogram - vBoWPos))
    DistNeg = np.sum(np.abs(histogram - vBoWNeg))
