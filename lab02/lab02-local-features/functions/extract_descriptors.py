...
def filter_keypoints(img, keypoints: np.array, patch_size=9):
    '''
    Inputs:
    ...
    # return keypoints where x and y are greater than patch_size/2 and less than img.shape - patch_size/2
    return keypoints[np.logical_and(np.logical_and(keypoints[:, 0] > patch_size/2, keypoints[:, 1] > patch_size/2),
                                    np.logical_and(keypoints[:, 0] < img.shape[1] - patch_size/2, keypoints[:, 1] < img.shape[0] - patch_size/2))]
...
def extract_patches(img, keypoints, patch_size=9):
    ...
    desc = np.take(img, ranges[:, None] * w + ranges + (keypoints[:, 1]
                   * w + keypoints[:, 0])[:, None, None])  # (q, patch_size, patch_size)
