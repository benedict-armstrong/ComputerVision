...
def Find2D3DCorrespondences(image_name, images, matches, registered_images):
    ...
    print(
        f'found {len(p3D_idxs)} points, {np.unique(np.array(p3D_idxs)).shape[0]} unique points')
...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
    ...
    starting_idx = len(points3D)
    points3D = np.append(points3D, new_points3D, 0)
    offset = starting_idx
    for im_name in corrs:
        idxs = np.arange(offset, offset + len(corrs[im_name]))
        images[im_name].Add3DCorrs(corrs[im_name], idxs)
        offset += len(corrs[im_name])
