...
def main():
    ...
    # for image_pair in itertools.combinations(image_names, 2):
    #     PlotImagePairMatches(images[image_pair[0]], images[image_pair[1]], matches[(
    ...
    for i, (R, t) in enumerate(possible_relative_poses):
        # set images poses as per hint in the assignment
        e_im1.SetPose(R, t)
        e_im2.SetPose(np.eye(3), np.zeros(3))

        # Triangulate points
        points3D, im1_corrs, im2_corrs = TriangulatePoints(
            K, e_im1, e_im2, e_matches)
    ...
    e_im1.SetPose(
        possible_relative_poses[best_pose][0], possible_relative_poses[best_pose][1])
    e_im2.SetPose(np.eye(3), np.zeros(3))
    ...
    while len(registered_images) < len(images):
        for image_name in images:
            ...
            image_kp_idxs, point3D_idxs = Find2D3DCorrespondences(
            ...
            print(
                f'Register image {image_name} from {len(image_kp_idxs)} correspondences')

            # Estimate new image pose
            R, t = EstimateImagePose(
                images[image_name].kps[image_kp_idxs], points3D[point3D_idxs], K)
